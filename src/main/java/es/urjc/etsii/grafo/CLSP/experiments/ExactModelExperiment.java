...
import es.urjc.etsii.grafo.CLSP.exact.CLSPExactModel;
...
public class ExactModelExperiment extends AbstractExperiment<CLSPSolution, CLSPInstance> {


    private final CLSPConfig config;

    public ExactModelExperiment(CLSPConfig config) {
    ...
    public List<Algorithm<CLSPSolution, CLSPInstance>> getAlgorithms() {
        // Experiment to test developing stuff
        var algorithms = new ArrayList<Algorithm<CLSPSolution, CLSPInstance>>();

        algorithms.add(new SimpleAlgorithm<>("Exact-Model", new CLSPExactModel(config)));
